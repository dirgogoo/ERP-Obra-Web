...
public class ItemEtapaController {
    ...
    public ResponseEntity <List<ItemEtapa>> findByEtapaObraId(@PathVariable Long id){
        List<ItemEtapa> obj = this.itemEtapaService.findAllbyEtapaObraIdAll(id);
       return ResponseEntity.ok().body(obj);
    }

    @PutMapping
    public ResponseEntity<Void> update( @RequestBody ItemEtapa obj) {
        this.itemEtapaService.update(obj);
